...
export function SutaVisualPanel({ visual, onAction, speaking = false }: { visual: SutaVisual | null; onAction?: (action: SutaAction) => void; speaking?: boolean }) {
  ...
  return <Shell sceneKey={sceneKey}><section className="flex h-full flex-col"><div className="flex-1 overflow-auto p-7"><Reveal delay={d1}>{visual.eyebrow && <span className="text-xs font-bold uppercase tracking-[0.18em] text-ansut-orange">{visual.eyebrow}</span>}<h3 className="mt-2 text-2xl font-semibold text-ansut-blue">{visual.title}</h3><p className="mt-3 leading-6 text-ansut-text-muted">{visual.summary}</p></Reveal>{visual.facts?.length ? <dl className="mt-6 grid gap-3">{visual.facts.slice(0, 3).map((fact, index) => <div key={fact.label} style={{ animationDelay: `calc(${d2} + ${index * 120}ms)` }} className="rounded-2xl bg-ansut-blue/[0.04] p-4 animate-[suta-scene-in_380ms_ease-out_both]"><dt className="text-sm font-medium text-ansut-blue">{fact.label}</dt>{fact.value && <dd className="mt-1 text-xs text-ansut-text-muted">{fact.value}</dd>}</div>)}</dl> : null}</div><ActionRow actions={visual.actions} onAction={onAction} delay={d3} /></section></Shell>;
}
