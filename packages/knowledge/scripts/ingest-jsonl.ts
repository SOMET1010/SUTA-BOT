...
/**
 * `npm run knowledge:ingest-jsonl -- <chemin-vers-corpus.jsonl>`
...
async function main() {
  ...
  if (!fileArg) {
    console.error("Usage : npm run knowledge:ingest-jsonl -- <chemin-vers-corpus.jsonl>");
    process.exitCode = 1;
    return;
  }

  const filePath = resolve(fileArg);
  console.log(`Ingestion du corpus JSONL : ${filePath}`);
